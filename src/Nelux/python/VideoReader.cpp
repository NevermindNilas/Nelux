...
void VideoReader::setRangeByFrames(int startFrame, int endFrame)
{
    ...
    const int total = (startFrame < 0 || endFrame < 0) ? exactRangeFrameCount() : 0;
    // Handle negative indices using the actual decoded frame count.
    if (startFrame < 0)
    {
...
void VideoReader::setRangeByTimestamps(double startTime, double endTime)
{
    ...
    if (!std::isfinite(startTime) || !std::isfinite(endTime))
        throw std::invalid_argument("Time range bounds must be finite.");
...
void VideoReader::setRangesByFrames(const std::vector<std::pair<int, int>>& ranges)
{
    ...
    const bool needsCount = std::any_of(ranges.begin(), ranges.end(),
        [](const auto& r) { return r.first < 0 || r.second < 0; });
    const int total = needsCount ? exactRangeFrameCount() : 0;
    ...
    const std::vector<std::pair<double, double>>& ranges)
{
    ...
    for (size_t i = 0; i < ranges.size(); ++i)
    {
        ...
        if (!std::isfinite(startTime) || !std::isfinite(endTime))
            throw std::invalid_argument(where + "time range bounds must be finite.");
...
void VideoReader::clearRanges()
{
    ...
    start_time = -1.0;
    end_time = -1.0;
    rangeTimeOrigin_.reset();
    rangeLastTimestamp_.reset();
    rangeTimestampError_ = false;
    rangeFinished_ = false;
...
int VideoReader::classifyAgainstActiveRange() const
{
    ...
    {
        // Only absorb floating-point roundoff from seconds conversion and
        // origin subtraction. A fixed nanosecond would move real boundaries
        // that a caller places just above/below a frame's timestamp.
        const double tolerance = 8 * std::numeric_limits<double>::epsilon() *
            std::max({1.0, std::abs(current_timestamp),
                      std::abs(rangeTimeOrigin_.value_or(0.0))});
        if (rangeTimestamp_ + tolerance < start_time)
            return -1;
        if (rangeTimestamp_ + tolerance >= end_time)
            return 1;
        return 0;
    }
...
bool VideoReader::advanceToNextSegment()
{
    ...
    // next() keeps the boundary frame buffered and reclassifies it against
    // each following segment. Decoding forward preserves its true ordinal,
    // including VFR, absent timestamps, and gaps within an open GOP.
    return true;
}
...
torch::Tensor VideoReader::decodeRangeFrame()
{
    ...
    if (!segments_.empty() && !segments_.front().byFrames)
    {
        ...
        {
            const bool first = !rangeTimeOrigin_;
            timestamp = underReaderLock([&](nelux::Decoder& d)
            {
                if (d.getCtx()->codec_id != AV_CODEC_ID_VP9)
                    return timestamp;
                const bool haveFrame = frame.defined() && frame.numel() > 0;
                if (!haveFrame && !rangeTimingDecoder_)
                    return timestamp;
                if (first || !rangeTimingDecoder_)
                {
                    // This is a timing sidecar only: output stays on NVDEC.
                    // Scale the unused software pixels down and keep its pool
                    // small. Unlike FPS inference it preserves VFR, duplicate
...
int VideoReader::exactRangeFrameCount()
{
    ...
    for (;;)
    {
        ...
        if (++count > std::numeric_limits<int>::max())
            throw std::overflow_error("Too many decoded frames for integer range bounds");
    }
...
{
    // Retry loop, because the file this reader reads is not fixed: reconfigure()
    // rewrites filePath under the lock, and the open below runs outside it. The
    // path is snapshotted under the lock, and the snapshot is re-checked against
    // the member before the finished decoder is published, so a reconfigure that
    // lands in that window makes this build again instead of publishing a
    // random-access decoder opened on the previous file. Every other constructor
    // argument here is written once in the VideoReader constructor and never
    // again, so only the path needs the snapshot.
    for (;;)
    {
        std::string path;
        {
        ...
        {
            // Guarded like underReaderLock's: decodeFrameAt() re-enters this
            // after a reconfigure stole the decoder, and it is already inside
            // its own gil_scoped_release by then.
...

        {
            ...
            // the decoder above was being built, and publishing now would
            // resurrect random access on a closed reader.
            if (!decoder)
                throw std::runtime_error("VideoReader is closed");
            if (rand_decoder)
                return; // another thread won the race; drop ours below
            if (filePath == path)
            {
        ...
        // reconfigure() pointed the reader somewhere else while this was
        // opening. Dropped outside the lock — the teardown joins threads and
        // must not block close() — and the loop opens the new file.
        NELUX_DEBUG("Discarding random-access decoder for '{}'; reader moved on",
                    path);
    }
}
...
torch::Tensor VideoReader::decodeFrameAt(double timestamp_seconds)
{
    ...
    py::gil_scoped_release release;
    std::unique_lock<std::shared_mutex> lk(lifecycleMu_, std::defer_lock);

    // Build-then-lock, retried: ensureRandDecoder() takes the lock itself to
    // publish (lifecycleMu_ is not recursive, so it cannot run under ours), and
    // reconfigure() takes the random-access decoder away under the same lock.
    // A single ensure-then-lock therefore has a window where the decoder this
    // call was promised is already gone, which surfaced as a spurious
    // "Random-access decoder not initialized" from a perfectly legal
    // frame_at() racing a reconfigure(). Rebuild instead of failing; a closed
    // reader still throws, out of ensureRandDecoder().
    std::shared_ptr<nelux::Decoder> rdec;
    for (;;)
    {
        ensureRandDecoder();
        lk.lock();
    ...
    //        threshold for sequential random access, so a long
...
VideoReader& VideoReader::iter()
{
    ...
    if (!segments_.empty())
    {
        // Without an ordinal index, neither frame/fps nor a container's VFR
        // flag proves the identity of a frame after a seek. Start from the
        // real beginning and count decoded frames, preserving display order.
        // Reopen a touched decoder: even a seek to zero can skip initial frames
        // in MPEG-TS/open GOPs. A fresh reader already has the right pre-roll.
        rewindForFreshIteration();
        currentIndex = 0;
        current_timestamp = 0.0;

        // Preserve iter()'s positioning contract: the first selected frame is
...
py::object VideoReader::next()
{
    ...
    if (rangeFinished_ && !segments_.empty())
    ...
    while (true)
    {
        // For frame bounds we already know the next ordinal. Finish before
        // decoding beyond the outpoint (which may lie in a damaged tail).
        if (!hasBufferedFrame && end_frame >= 0 && currentIndex > end_frame)
        {
...
void VideoReader::reconfigure(const std::string& newFilePath)
{
    ...
    // ONE exclusive critical section for the switch itself. Reconfiguring the
    // decoder and rewriting the state that describes it (properties, filePath,
    // the output tensor) has to be atomic with respect to everything else: a
    // decode holding the lock reads exactly those fields, and close() must not
    // be able to tear the decoder down half-way through. Taking the old
    // random-access decoder away here too mirrors the handoff close() performs.
    //
    // The reader's own counters are the exception: they are serialised by the
    // GIL (iter()/next()/reset() touch them holding nothing else), and
    // underReaderLock below drops it. Reset them here, before the switch —
    // a reconfigure that throws leaves the reader unusable anyway, so zeroing
    // early costs nothing and keeps every write to these fields on one
    // discipline.
    currentIndex = 0;
    current_timestamp = 0.0;
    nvdecTimestampOffset_ = 0.0;
