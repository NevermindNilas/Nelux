...
def main() -> int:
    # Nothing is added to this process's DLL search path on purpose. The wheel
    # bundles FFmpeg next to the extension and nelux/__init__.py add_dll_directory's
    # its own package directory, so a correct wheel loads with no help. Handing
    # the loader FFMPEG_BIN (the build tree, which holds every av*/sw* runtime
    # DLL) would satisfy the extension's delay-load imports from outside the
    # wheel and hide exactly the failure this test exists to catch -- the
    # workflows are careful not to put that directory on PATH for the same
    # reason. FFMPEG_BIN is still read by _resolve_ffmpeg(), because the clip is
    # generated by running ffmpeg.exe as a subprocess and a subprocess resolves
    # its own dependencies from its own directory.
