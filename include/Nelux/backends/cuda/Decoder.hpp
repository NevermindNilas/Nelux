...
class Decoder : public nelux::Decoder
{
  public:
     ...
     * @param meanRGB Mean values for normalization [R, G, B] (e.g., [0.485, 0.456,
     * 0.406] for ImageNet)
     * @param stdRGB Standard deviation values [R, G, B] (e.g., [0.229, 0.224, 0.225]
     * for ImageNet)
     */
    void setMLOutputMode(bool enable, const float meanRGB[3] = nullptr,
                         const float stdRGB[3] = nullptr);

    /**
     * @brief Check if ML output mode is enabled
     */
    bool isMLOutputMode() const
    {
        return mlOutputMode_;
    }
     ...
     */
    void setMLUseFP16(bool useFP16)
    {
        mlUseFP16_ = useFP16;
    }
